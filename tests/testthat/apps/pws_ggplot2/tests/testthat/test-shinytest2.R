...
test_that("{shinytest2} plot_with_settings: download ggplot2 modal", {
  ...
  date <- strftime(Sys.time(), format = "%Y%m%d")
  testthat::expect_true(
    grepl(paste0("plot_", date), app$get_value(input = "plot_with_settings-modal_downbutton-file_name"))
  )
  ...
  testthat::expect_equal(isolate(vals$output$`plot_with_settings-plot_modal`$height), 1200)
  testthat::expect_equal(isolate(vals$output$`plot_with_settings-plot_modal`$width), 750)
...
test_that("{shinytest2} plot_with_settings: width warning", {
  ...
  # starts out visible
  expect_equal(app$get_js("$('#plot_with_settings-width_warning').children().length"), 1)
  vals <- app$get_values()
  expect_equal(
    vals$output$`plot_with_settings-width_warning`$html[1],
    paste0(
  ...
  expect_equal(app$get_js("$('#plot_with_settings-width_warning').children().length"), 0)
